...
A collab represents the relationship between a creator and a channel (brand). Each collab includes discount settings, affiliate tracking, and tier classification.
...
| `channel` | String | Channel/brand reference |
| `channelName` | String | Human-readable channel name |
| `collabTier` | String | Tier ID reference |
...
| `drops` | Array[String] | Array of drop IDs |
| `metadata` | Object | Shopify integration metadata |
| `custom` | Object | Custom fields (brand-specific) |
...
Collabs belong to campaigns through their tier:
