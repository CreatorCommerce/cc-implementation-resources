...
Show a persistent banner when shopping through a creator:
...
{% if cc_creator %}
  <div class="cc-shopping-banner">
    {% if cc_creator.data.value['cc-creator-profile-picture'].url %}
      <img 
        src="{{ cc_creator.data.value['cc-creator-profile-picture'].url}}" 
    ...
    <span>
      ...
      {% if cc_creator['cc-collab-discount-amount'] %}
        · {{ cc_creator['cc-collab-discount-amount'] }}% Off Everything
      {% endif %}
    </span>
    <a href="{{ cc_creator.data.value['cc-collab-affiliate-link'] }}">View Shop →</a>
  </div>
{% endif %}
```
...
{% if cc_creator %}
  <div class="cc-footer-cta">
    ...
    <a href="{{ cc_creator.data.value['cc-collab-affiliate-link'] }}" class="btn">
