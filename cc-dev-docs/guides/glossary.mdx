...
Use these as the preferred primary terms across the docs:

| Preferred term | Use when | Acceptable aliases |
|----------------|----------|--------------------|
| Creator | Referring to the person in the CC data model | Partner, affiliate, influencer, ambassador, practitioner when audience context specifically calls for it |
| Channel | Referring to the merchant's Shopify store in technical or API contexts | Brand, merchant, store in audience-facing prose |
| Collab | Referring to the creator-channel relationship object | Collaboration |
| Campaign | Referring to the routing and grouping object in the API | Destination in merchant-facing UX where helpful |
| Co-branded storefront | Referring to the personalized shopping surface | Creator page, landing page, partner page where context narrows the meaning |

If a page needs one canonical technical term, prefer `creator`, `channel`, `collab`, and `campaign`.
...
### Tier (Collab Tier)
An assigned tier level to a collab that defines the discount rates, relationship type, and classification level (e.g., "Ambassadors", "Influencers", "Partners"). Tiers determine the benefits and permissions for creators within a collaboration and may be associated with campaigns (also known as Destinations).
...
A marketing initiative that groups multiple collab tiers together. Also referred to as a **Destination**. Campaigns can have brand pages, product pages, and redirect URL templates. They serve as the routing mechanism that determines where traffic from different tiers is directed.

**Tiers vs Destinations:**
...
Multiple tiers can belong to the same campaign/destination, allowing you to organize different creator relationship levels under a unified marketing initiative.
...
### Safelinks
A discount link format that automatically applies a discount code when a customer clicks through, without requiring them to manually enter the code at checkout. Safelinks provide a seamless shopping experience by embedding the discount directly in the URL.

**How CreatorCommerce Supports Safelinks:**
...
- [API Overview](/references/api-overview) - Capability map for the Unified API
- [Unified API Reference](/references/unified-api-reference) - Endpoint-level documentation
- [Co-Branded Storefronts](/guides/co-branded-storefronts) - Building personalized experiences
- [Build with CreatorCommerce](/guides/getting-started) - Platform overview and setup
